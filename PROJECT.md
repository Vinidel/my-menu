...
- **Delivered:** App Skeleton (Next.js, Tailwind, shadcn, Vitest, `/` and `/admin` placeholders), Employee Auth (Supabase email/password login, protected `/admin`, `/admin/login`, logout), Employee Orders Dashboard (`/admin` summary/list/details/status progression), Admin Orders Dashboard UX Enhancements (status-first sorting on `/admin`, mobile accordion details, single-expand mobile behavior), Admin Orders Dashboard Polling (TanStack Query polling on `/admin`, protected `GET /api/admin/orders`, hidden-tab pause/resume behavior), Admin Order Total Amount Display (`/admin` order details total with pt-BR currency formatting, legacy `Indisponível` fallback, snapshot-based totals), Supabase `orders` schema + seed and DB-enforced status transitions, Customer Order Submission (`/` menu/cart/checkout, `public.customers`, `/api/orders`, service-role submission path), Customer Cart Visibility / Feedback (`Carrinho` naming, explicit cart entry labels, add-to-cart feedback highlight, mobile sticky cart tabs), API Orders Anti-Abuse (`POST /api/orders` rate limiting, `429` + `Retry-After`, hashed source keys, degrade-open fallback), Order Item Extras / Customization (menu extras in JSON, customer extras selection/editing, server-validated extras snapshots, `/admin` extras display), and Order Payment Method Selection (required `modo de pagamento` on checkout, `orders.payment_method`, `/admin` payment method details display).
- **Docs:** Feature briefs in `docs/briefs/`; delivery notes in `docs/employee-auth.md`, `docs/employee-orders-dashboard.md`, `docs/admin-orders-dashboard-ux-mobile-and-status-sorting.md`, `docs/admin-orders-dashboard-polling-tanstack-query.md`, `docs/admin-order-total-amount-display.md`, `docs/customer-order-submission.md`, `docs/customer-cart-visibility-feedback.md`, `docs/api-orders-anti-abuse.md`, `docs/order-item-extras-customization.md`, and `docs/order-payment-method-selection.md`. Implementation and hardening notes in `docs/implementation-notes.md` and `docs/hardening-notes.md`.
...
- **Customer flow:** Public menu page (data from JSON, including optional additive extras) → tabbed `Cardápio` / `Carrinho` UX with explicit cart count and add-feedback highlight (mobile sticky cart tabs) → cart/selection (line items may include extras) → checkout form (name, email, phone required; optional notes; required `modo de pagamento`) → `POST /api/orders` (server-only service-role write path + anti-abuse rate limiting + server extras/pricing validation + payment method validation) → order submission. Orders store backward-compatible item pricing snapshots for reliable admin totals and persist canonical `payment_method`.
- **Employee flow:** Login (Supabase Auth, email + password) → `/admin` dashboard (status-first sorted list, mobile accordion on `<768px`, TanStack Query polling auto-refresh via protected `GET /api/admin/orders`) → update order status. Only authenticated users can access the owner/employee area.
...
├── docs/                ← briefs/, critique.md, implementation-notes.md, hardening-notes.md, employee-auth.md, employee-orders-dashboard.md, admin-orders-dashboard-ux-mobile-and-status-sorting.md, admin-orders-dashboard-polling-tanstack-query.md, admin-order-total-amount-display.md, customer-order-submission.md, customer-cart-visibility-feedback.md, api-orders-anti-abuse.md, order-item-extras-customization.md, order-payment-method-selection.md (feature docs)
