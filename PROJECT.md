...
- **Employee flow:** Login (Supabase Auth, email + password) → `/admin` dashboard (status-first sorted list, mobile accordion on `<768px`) → update order status. Only authenticated users can access the owner/employee area.
...
├── docs/                ← briefs/, critique.md, implementation-notes.md, hardening-notes.md, employee-auth.md, employee-orders-dashboard.md, admin-orders-dashboard-ux-mobile-and-status-sorting.md, customer-order-submission.md, api-orders-anti-abuse.md, order-item-extras-customization.md (feature docs)
