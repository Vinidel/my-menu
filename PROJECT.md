...
- **Customer flow:** Public menu page (data from JSON, including optional additive extras) → tabbed cardápio/pedido UX → cart/selection (line items may include extras) → checkout form (name, email, phone required; optional notes) → `POST /api/orders` (server-only service-role write path + anti-abuse rate limiting + server extras/pricing validation) → order submission. Orders store backward-compatible item pricing snapshots for reliable admin totals.
