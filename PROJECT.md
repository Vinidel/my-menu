...
- **Employee flow:** Login (Supabase Auth, email + password) → `/admin` dashboard (status-first sorted list, mobile accordion on `<768px`, TanStack Query polling auto-refresh via protected `GET /api/admin/orders`) → update order status. Menu imports are processed asynchronously via Supabase queue worker; admin UI poller only refreshes status. Only authenticated users can access the owner/employee area.
