...
alter table public.orders
drop constraint if exists orders_delivery_status_matches_fulfillment_check;

alter table public.orders
add constraint orders_delivery_status_matches_fulfillment_check
check (
  status <> 'saiu_para_entrega'
  or fulfillment_type = 'entrega'
);
...
begin
  ...
  if old.status = 'em_preparo' and coalesce(old.fulfillment_type, '') <> 'entrega' and new.status = 'entregue' then
    return new;
  end if;
...
comment on function public.enforce_order_status_transition() is
  'Impede transições de status fora do fluxo aguardando_confirmacao -> em_preparo -> saiu_para_entrega -> entregue para entrega, mantendo aguardando_confirmacao -> em_preparo -> entregue para retirada.';
